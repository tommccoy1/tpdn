...
def train_filter(training_set, encoder, decoder, tpr_enc, tpr_dec, encoder_optimizer, decoder_optimizer, tpr_enc_optimizer, tpr_dec_optimizer, criterion, input_to_output, parsing_fn, role_fn):
    loss = 0
    
    encoding = encoder(training_set)
    tpr_enc_encoding = tpr_enc(training_set)
    tpr_dec_encoding = tpr_dec([input_to_output(x) for x in training_set])   

    #print(training_set)
    #14/0
 
    # Get the decoder's outputs outputs for these inputs
    #logits = decoder(encoding - tpr_enc_encoding, 
    logits = decoder(tpr_dec_encoding.detach(), 
    ...
    tpr_enc_optimizer.zero_grad()
    tpr_dec_optimizer.zero_grad()
    ...
    for index, logit in enumerate(logits):
        if use_cuda:
            ...
            loss += nn.MSELoss()(tpr_enc_encoding + tpr_dec_encoding, encoding)
            #loss += nn.MSELoss()(torch.norm(tpr_enc_encoding + tpr_dec_encoding), torch.norm(encoding))
    ...
    tpr_enc_optimizer.step()
    tpr_dec_optimizer.step()
...
#    input_to_output: function that maps input sequences to correct outputs
def dev_loss_filter(encoder, decoder, tpr_enc, tpr_dec, criterion, dev_set, input_to_output, parsing_fn, role_fn):
    dev_loss_val = 0
   
    for dev_elt in dev_set:
        logits = decoder(encoder(dev_elt) - tpr_enc(dev_elt), 
...
def train_iters_filter(encoder, decoder, tpr_enc, tpr_dec, train_data, dev_data, file_prefix, input_to_output, max_epochs=100, patience=1, print_every=1000, learning_rate=0.001, batch_size=32, parsing_fn=None, role_fn=None, train_enc=True, train_dec=True, save_intermediate=False):
    print_loss_total = 0
    
    # Train using Adam
    encoder_optimizer = optim.Adam(encoder.parameters(), lr=learning_rate)
    decoder_optimizer = optim.Adam(decoder.parameters(), lr=learning_rate)
    tpr_enc_optimizer = optim.Adam(tpr_enc.parameters(), lr=learning_rate)
    tpr_dec_optimizer = optim.Adam(tpr_dec.parameters(), lr=learning_rate)
    ...
    for epoch in range(max_epochs):
       ...
       # Iterate over batches
        for batch, training_set in enumerate(training_sets):
            ...
            loss = train_filter(training_set, encoder, decoder, tpr_enc, tpr_dec, encoder_optimizer, decoder_optimizer, tpr_enc_optimizer, tpr_dec_optimizer, criterion, input_to_output, parsing_fn, role_fn)            
            # Print an update and save the weights every print_every iterations
            if batch % print_every == 0:
                this_loss = dev_loss_filter(encoder, decoder, tpr_enc, tpr_dec, criterion, dev_data, input_to_output, parsing_fn, role_fn)
                progress_file.write(str(epoch) + "\t" + str(batch) + "\t" + str(this_loss.item()) + "\n")
                print(str(epoch) + "\t" + str(batch) + "\t" + str(this_loss.item()) + "\n")
                if this_loss.data[0] < best_loss:
                    ...
                    if train_dec:
                        ...
                        torch.save(decoder.state_dict(), "models/decoder_" + file_prefix + ".weights")
                        torch.save(tpr_dec.state_dict(), "models/tpr_dec_" + file_prefix + ".weights")     
                        torch.save(tpr_enc.state_dict(), "models/tpr_enc_" + file_prefix + ".weights")
        # Early stopping
...
# Training a TPDN for a single training example
def train_tpr_double(batch_set, tpr_encoder, tpr_encoder2, tpr_optimizer, tpr_optimizer2, criterion):
    ...
    for training_set in batch_set:
        ...
        # Find the output for this input
        tpr_encoder_output = tpr_encoder(input_fillers)
        tpr_encoder_output2 = tpr_encoder2(input_fillers)

        # Find the loss associated with this output
        #loss += criterion(tpr_encoder_output.unsqueeze(0), target_variable)
        loss += criterion(tpr_encoder_output + tpr_encoder_output2, target_variable)
    ...
    tpr_optimizer2.step()
    tpr_encoder_output = tpr_encoder_output.detach()
    tpr_encoder_output2 = tpr_encoder_output2.detach()
    
    # Return the loss
    return loss.data.item()

# Training a TPDN for multiple iterations
def train_iters_tpr_double(train_data, dev_data, tpr_encoder, tpr_encoder2, n_epochs, print_every=1000, learning_rate=0.001, batch_size=5, weight_file=None, patience=10):
    # The amount of loss accumulated between print updates
    print_loss_total = 0
    
    # The optimization algorithm; could use SGD instead of Adam
    tpr_optimizer = optim.Adam(tpr_encoder.parameters(), lr=learning_rate)
    tpr_optimizer2 = optim.Adam(tpr_encoder2.parameters(), lr=learning_rate) 
    ...
    for epoch in range(n_epochs):
        ...
        for batch in range(len(train_data)//batch_size):
            ...
            loss = train_tpr_double(batch_set, tpr_encoder, tpr_encoder2, tpr_optimizer, tpr_optimizer2, criterion)       
            ...
            if batch % print_every == 0:
                ...
                for i in range(len(dev_data)):  
                    total_mse += torch.mean(torch.pow(tpr_encoder(dev_data[i][0]).data + tpr_encoder2(dev_data[i][0]).data - dev_data[i][1].data, 2))
                ...
                if print_loss < best_loss:
                    ...
                    best_loss = print_loss
                    torch.save(tpr_encoder.state_dict(), weight_file)
                    torch.save(tpr_encoder2.state_dict(), weight_file + "double_second")
                else:
                    count_not_improved += 1
                    if count_not_improved == patience: 
                        training_done = 1
...
def train_tpr_unbind(training_set, input_to_output, tpr_decoder, tpr_optimizer, criterion, role_fn=None):
    ...
    # Iterate over this batch
    input_fillers = training_set[0] #[list(x.data.cpu().numpy()) for x in training_set[0]] # The list of fillers for the input
    enc = training_set[1] # The mystery vector associated with this input

    #print("inp", training_set)
    #print(role_fn(input_fillers))
    #14/0
    ...
    for index, logit in enumerate(logits):
        if use_cuda:
            loss += criterion(logit, Variable(torch.LongTensor([[x for x in training_set[0]]])).cuda().transpose(0,2)[index].view(-1))
            ...
            loss += criterion(logit, Variable(torch.LongTensor([[x for x in input_fillers]])).transpose(0,2)[index].view(-1))
...
def dev_loss_unbind(decoder, criterion, dev_set, input_to_output, parsing_fn, role_fn):
    ...
    for dev_elt in dev_set:
        ...
        input_fillers = dev_elt[0] #[list(x.data.cpu().numpy()) for x in dev_elt[0]]
        
        for index, logit in enumerate(logits):
                    if use_cuda: 
                        dev_loss_val += criterion(logit, Variable(torch.LongTensor([[x for x in input_fillers]])).cuda().transpose(0,2)[index].view(-1))
			...
			#        loss += criterion(logit, Variable(torch.LongTensor([[input_to_output(x) for x in training_set[0]]])).cuda().transpose(0,2)[index].view(-1))
                        ...
                        dev_loss_val += criterion(logit, Variable(torch.LongTensor([[x for x in input_fillers]])).transpose(0,2)[index].view(-1))
...
#   criterion: the loss function
#   input_to_output: function that maps input sequences to correct outputs
def train_stages(training_set, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, input_to_output, parsing_fn, role_fn, current_length=None):
    loss = 0
    ...
    logits = decoder(encoder(training_set), 
                     output_len=len(training_set[0]), 
                     tree=parsing_fn(training_set),
                     role_list=role_fn(training_set))[:current_length]
...
#    dev_set: the development set
#    input_to_output: function that maps input sequences to correct outputs
def dev_loss_stages(encoder, decoder, criterion, dev_set, input_to_output, parsing_fn, role_fn, current_length=None):
    dev_loss_val = 0
    ...
    for dev_elt in dev_set:
        logits = decoder(encoder(dev_elt), 
                         output_len=len(dev_elt[0]),
                         tree=parsing_fn(dev_elt), 
                         role_list=role_fn(dev_elt))[:current_length]
...
#    learning_rate: learning rate
#    batch_size: batch_size
def train_iters_stages(encoder, decoder, train_data, dev_data, file_prefix, input_to_output, max_epochs=100, patience=1, print_every=1000, learning_rate=0.001, batch_size=32, parsing_fn=None, role_fn=None, train_enc=True, train_dec=True, save_intermediate=False, max_length=6):
    print_loss_total = 0
        ...
        for epoch in range(max_epochs):
           ...
           # Iterate over batches
            for batch, training_set in enumerate(training_sets):
                ...
                # Train for this batch
                loss = train_stages(training_set, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, input_to_output, parsing_fn, role_fn, current_length=current_length)            
                # Print an update and save the weights every print_every iterations
                if batch % print_every == 0:
                    this_loss = dev_loss_stages(encoder, decoder, criterion, dev_data, input_to_output, parsing_fn, role_fn, current_length=current_length)
                    progress_file.write(str(epoch) + "\t" + str(batch) + "\t" + str(this_loss.item()) + "\n")
                    print(str(epoch) + "\t" + str(batch) + "\t" + str(this_loss.item()) + "\n")
                    if this_loss.data[0] < best_loss:
                        improved_this_epoch = 1
