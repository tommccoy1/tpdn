...
parser.add_argument("--n_hidden_dec", help="number of hidden layers in the decoder", type=int, default=None)
parser.add_argument("--decomp_type", help="TPBN or TPUN", type=str, default="tpbn")
parser.add_argument("--double_role_scheme", help="second role scheme to add in", type=str, default=None)
args = parser.parse_args()

prefix_double = ""
if args.double_role_scheme is not None:
	prefix_double = "_" + args.double_role_scheme

# Create the logfile
if args.decomp_type == "tpbn":
	if args.final_linear != "True":
		results_page = open("logs/" + args.data_prefix.split("/")[-1] + str(args.role_prefix).split("/")[-1] + str(args.role_scheme) + prefix_double + ".filler" + str(args.filler_dim) + ".role" + str(args.role_dim) + ".tpr_encomp.nf", "w")
	else:
		results_page = open("logs/" + args.data_prefix.split("/")[-1] + str(args.role_prefix).split("/")[-1] + str(args.role_scheme) + prefix_double + ".filler" + str(args.filler_dim) + ".role" + str(args.role_dim) + "." + str(args.embed_squeeze) + ".tpr_encomp", "w")

else:
	if args.final_linear != "True":
		results_page = open("logs/" + args.data_prefix.split("/")[-1] + str(args.role_prefix).split("/")[-1] + str(args.role_scheme) + prefix_double + ".filler" + str(args.filler_dim) + ".role" + str(args.role_dim) + ".tpr_decomp.nf", "w")
	else:
		results_page = open("logs/" + args.data_prefix.split("/")[-1] + str(args.role_prefix).split("/")[-1] + str(args.role_scheme) + prefix_double + ".filler" + str(args.filler_dim) + ".role" + str(args.role_dim) + "." + str(args.embed_squeeze) + ".tpr_decomp", "w")
...
if args.embedding_file is not None:
	weights_matrix = create_embedding_dictionary(args.embedding_file, args.filler_dim, filler2index, index2filler, emb_squeeze=args.embed_squeeze, unseen_words=args.unseen_words)
...
if args.decomp_type == "tpbn":
	if args.double_role_scheme is not None:
		...
		if args.final_linear == "True":
			...
			tpr_encoder2 = TensorProductEncoder(role_scheme=args.double_role_scheme, n_fillers=vocab_size, final_layer_width=args.hidden_size, filler_dim=args.filler_dim, role_dim=args.role_dim, pretrained_embeddings=weights_matrix, embedder_squeeze=args.embed_squeeze, max_length=max_length, role_filea=role_filea, filler_filea=filler_filea, role_fileb=role_fileb, filler_fileb=filler_fileb,role_filec=role_filec, filler_filec=filler_filec,role_filed=role_filed, filler_filed=filler_filed)
		else:
			tpr_encoder = TensorProductEncoder(role_scheme=args.role_scheme, n_fillers=vocab_size, final_layer_width=None, filler_dim=args.filler_dim, role_dim=args.role_dim, pretrained_embeddings=weights_matrix, embedder_squeeze=args.embed_squeeze, max_length=max_length, role_filea=role_filea, filler_filea=filler_filea, role_fileb=role_fileb, filler_fileb=filler_fileb,role_filec=role_filec, filler_filec=filler_filec,role_filed=role_filed, filler_filed=filler_filed)
			tpr_encoder2 = TensorProductEncoder(role_scheme=args.double_role_scheme, n_fillers=vocab_size, final_layer_width=None, filler_dim=args.filler_dim, role_dim=args.role_dim, pretrained_embeddings=weights_matrix, embedder_squeeze=args.embed_squeeze, max_length=max_length, role_filea=role_filea, filler_filea=filler_filea, role_fileb=role_fileb, filler_fileb=filler_fileb,role_filec=role_filec, filler_filec=filler_filec,role_filed=role_filed, filler_filed=filler_filed)

		tpr_encoder = tpr_encoder.to(device=device)
		tpr_encoder2 = tpr_encoder2.to(device=device) 
		...
		if args.train == "True":
			end_loss = train_iters_tpr_double(train_set, dev_set, tpr_encoder, tpr_encoder2, 100, print_every=1000//32, learning_rate = 0.001, weight_file="models/" + args.data_prefix + str(args.role_prefix) + str(args.role_scheme) + prefix_double + ".tpr_enc", batch_size=32)
	
		# Load the trained TPDn
		tpr_encoder.load_state_dict(torch.load("models/" + args.data_prefix + str(args.role_prefix) + str(args.role_scheme) + prefix_double + ".tpr_enc"))
		tpr_encoder2.load_state_dict(torch.load("models/" + args.data_prefix + str(args.role_prefix) + str(args.role_scheme) + prefix_double + ".tpr_encdouble_second")) 
		...
		for i in range(len(test_data)): 
			encoding = tpr_encoder(test_data[i][0]) + tpr_encoder2(test_data[i][0])
		...
		if args.test_decoder == "True":
			correct, total = score_double(tpr_encoder, tpr_encoder2, decoder, batchify([x[0] for x in test_set], 1), input_to_output, parsing_fn, role_fn)
	...
	args.role_prefix = str(args.role_prefix).split("/")[-1]
	if args.train == "True":
		end_loss = train_iters_tpr_unbind(train_set, dev_set, tpr_decoder, 100, input_to_output, print_every=1000//32, learning_rate = 0.001, weight_file="models/" + args.data_prefix + str(args.role_prefix) + str(args.role_scheme) + ".tpr_dec", batch_size=32, role_fn=role_fn)
